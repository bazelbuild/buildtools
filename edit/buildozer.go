...
func executeCommandsInFile(
	...
	// The "file" reference was updated if there were changes to the file.
	if changed {
		return f, nil
	}
	return nil, nil
...
func ExecuteCommandsOnInlineFile(fileContent []byte, commands []string) ([]byte, error) {
	...
	f = RemoveEmptyPackage(f)
	f = RemoveEmptyUseRepoCalls(f)
	outputFileContent, err := buildifier.Buildify(&opts, f)
	if err != nil {
		return nil, err
	}
...
// groupCommands parses and groups up provided commands per target.
// returns error if there is anything invalid or unexpected in the input commands.
func groupCommandsForInlineFile(commands []string, opts Options) ([]commandsForTarget, *string, error) {
	...
	// AppendCommandsFromReader will return commands based on a file in current dir (which does not exist).
	err := appendCommandsFromReader(&opts, commandReader, commandsByFile, []string{})
	if err != nil || len(commandsByFile) != 1 {
		return nil, nil, fmt.Errorf("error parsing commands %s", err)
	}
	if len(commandsByFile) != 1 {
		return nil, nil, fmt.Errorf("invlid input commands, commands resolved into modifying multiple different files")
	}
	for filepath, commandsForTargets := range commandsByFile {
		for i := range commandsForTargets {
			...
			case 1: // No-op
			case 2:
				// Ignores anything prefixing ":" character since inline changes only considers target name.
