...
`//MODULE.bazel:all`):
  ...
  * `use_repo_add <use_extension variable name> <repo(s)>`:
    ...
    - Mapped names: `bar=baz` imports the repository `baz` under the local name
      `bar` and generates `use_repo(ext, bar = "baz")`. A mapping whose local
      name equals its value (`foo=foo`) is simplified to `foo`.
    Repositories that are already imported (matched by local name or by
    repository value) are left untouched, so existing mappings are never
    overwritten. Buildozer does not check that mapped local names are valid
    Starlark identifiers.
  * `use_repo_set <use_extension variable name> <repo(s)>`:
    Like `use_repo_add`, but replaces any existing import that shares a local
    name or repository value with one of the given repositories. Use this to
    rename or re-map repositories. Accepts the same repository specifications as
    `use_repo_add`.
  ...
  * `use_repo_add [dev] <extension .bzl file> <extension name> <repo(s)>`:
    ...
    formats as the first variant.
  * `use_repo_set [dev] <extension .bzl file> <extension name> <repo(s)>`:
    Like the `use_repo_add` variant above, but replaces existing imports that
    conflict with the given repositories (see `use_repo_set` above).
